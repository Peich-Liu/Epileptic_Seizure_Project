...
class PandasTSData(BaseData):
    ...
    def load_all(self, root_dir, file_list=None, pattern=None):
        ...
        data_paths = [p for p in selected_paths if os.path.isfile(p) and "_eegs_df.pickle" in p]
        label_paths = [p for p in selected_paths if os.path.isfile(p) and "_labels_df.pickle" in p]
        if len(data_paths) == 0 or len(label_paths) == 0:
            raise Exception("No df files found using pattern: '{}'".format(pattern))

        all_df, labels_df = self.load_single(data_paths[0], label_paths[0])  # a single file contains dataset
    ...
    def load_single(self, data_path, label_path):
        ...
        # Then concatenate into a (num_samples * seq_len, feat_dim) dataframe, with multiple rows corresponding to the
        # sample index (i.e. the same scheme as all datasets in this project)
        df = pd.concat((pd.DataFrame({col: df.loc[df.index[row], col] for col in df.columns}).reset_index(drop=True)
            .set_index(pd.Series(lengths[row, 0]*[df.index[row]])) for row in range(df.shape[0])), axis=0)
